...
![Platform](https://img.shields.io/badge/Platform-LPC2129%20%7C%20ESP8266-blue)
![Language](https://img.shields.io/badge/Language-Embedded%20C%20%7C%20C%2B%2B-orange)
...
* **Standard 5V/3.3V Power Supply**

## 💻 Software & Toolchain
* **Keil µVision** (For compiling the bare-metal C firmware for LPC2129)
* **Arduino Web Editor / IDE** (For flashing the ESP8266 bridge)
* **Arduino IoT Cloud** (For the remote MQTT dashboard)
...
| ESP8266 (SoftwareSerial) | LPC2129 (Hardware UART0) | Function |
| :--- | :--- | :--- |
| D2 (GPIO 4) - TX | RXD0 (Pin 34) | Transmits cloud commands ('A', 'a', etc.) |
| D1 (GPIO 5) - RX | TXD0 (Pin 33) | Receives debug/status confirmations |
...
2. Create a new "Thing" and bind it to your ESP8266 NodeMCU device.
...
2. Include `<SoftwareSerial.h>` and initialize a virtual serial port on pins D1 and D2.
3. Map the state of `L1`-`L4` inside the auto-generated callback functions to transmit specific character bytes over `SoftwareSerial` (e.g., `if (L1 == true) { Serial2.print('A'); }`).
...
1. Wire the common ground between the LPC2129, ESP8266, and your power supply.
...
4. Toggle a switch. The command is routed over MQTT to the ESP8266, bridged via UART to the LPC2129, and physically executed by the relays within milliseconds.
